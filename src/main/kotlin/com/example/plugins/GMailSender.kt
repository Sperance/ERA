...
class GMailSender :
    ...
    override fun getPasswordAuthentication(): PasswordAuthentication {
        return PasswordAuthentication("era.hairs@gmail.com", "qfya ekvg xkva bspy")
    }

    @Synchronized
    @Throws(Exception::class)
    fun sendMail(subject: String?, body: String, recipients: String) {
        printTextLog("[GMailSender::sendMail] Адресат: $recipients Тема: $subject")
        try {
            val message = MimeMessage(session)
            val handler = DataHandler(ByteArrayDataSource(body.toByteArray(), "text/plain"))
            message.sender = InternetAddress("era.hairs@gmail.com")
